...
(function($) {
	$.tagator = function (element, options) {
		...
		plugin.init = function () {
			...
			$(input_element).width(80);
			$(input_element).attr('autocomplete', 'false');
			$(input_element).attr('name', 'input_tagName');
			...
			$(box_element).bind('focus', function (e) {
				...
				showOptions();
				$(input_element).focus();
				$(box_element).removeClass('options-hidden');
			});
			// box element
			$(input_element).bind('blur', function (e) {
				e.preventDefault();
				e.stopPropagation();
				$(box_element).addClass('options-hidden');
			});
		...
		var resizeInput = function () {
			...
			$(input_element).css({ width: ($(textlength_element).width() + 80) + 'px' });
		};
		...
		// SET DefaultTags LIST
		plugin.defaultTags = function (defaultTags) {
			plugin.settings.defaultTags = defaultTags !== undefined ? defaultTags : [];
		};
		...
		var removeTag = function (text) {
			...
			var tagIdsAfter = [];
			$.each(tagsAfter,function(i){
    			$.each(plugin.settings.autocomplete,function(item,value){
    				if(value.text == tagsAfter[i]){
    					tagIdsAfter.push(value.id);
    				}
    			});
			});
			$(element).attr("data-ids",tagIdsAfter.join(','));
		...
		var addTag = function (text,id) {
			//console.log(text,id);
			if (id != undefined && id !="" && !hasTag(text)) {
				$(element).val($(element).val() + ($(element).val() !== '' ? ',' : '') + text);
				$(element).attr("data-ids",$(element).attr("data-ids") + ($(element).attr("data-ids") !== '' ? ',' : '') + id);
				$(element).trigger('change');
			}
			$(input_element).val('');
			//box_element.focus();
		...
		var searchOptions = function () {
			...
			if (input_element.value.replace(/\s/g, '') !== '' || plugin.settings.showAllOptionsOnFocus) {
				...
				$.each(plugin.settings.autocomplete, function (key, value) {
					if (value.text.toLowerCase().indexOf(input_element.value.toLowerCase()) !== -1) {
						if (!hasTag(value.text)) {
		...
		var createOption = function (text, index) {
			...
			$(option).data('text', text.text);
			$(option).data('data-id', text.id);
			$(option).html(text.text);
		...
		var selectOption = function () {
			var optionElement = $(options_element).find('.' + plugin.settings.prefix + 'option').eq(selected_index);
			addTag(optionElement.data('text'),optionElement.data('data-id'));
			$(input_element).blur();
		};
