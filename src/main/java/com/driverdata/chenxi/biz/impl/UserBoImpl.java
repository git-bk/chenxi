...
import com.driverdata.chenxi.dal.mapper.RoleDoMapperExt;
import com.driverdata.chenxi.dal.mapper.UserDoMapperExt;
import com.driverdata.chenxi.dal.mapper.UserRoleDoMapperExt;
import com.driverdata.chenxi.dal.mapper.ValidateCodeDoMapperExt;
...
public class UserBoImpl implements UserBo {
    ...
    @Autowired
    private ValidateCodeDoMapperExt           validateCodeDoMapperExt;

    @Autowired
    private com.driverdata.chenxi.biz.ImageBo imageBo;
    ...
    public void sendMobileValidateCode(String mobile) throws ChanceValidateException {
        ...
        ValidateCodeDo record = new ValidateCodeDo();
        record.setCode(code);
        record.setPhone(mobile);
        record.setGmtCreate(new Date());
        record.setGmtModified(new Date());
        ValidateCodeDoExample example = new ValidateCodeDoExample();
        example.createCriteria().andPhoneEqualTo(mobile).andIsDeletedEqualTo("n");
        List<ValidateCodeDo> rslist = validateCodeDoMapperExt.selectByExample(example);
        if (rslist != null && rslist.size() > 0) {
            record.setId(rslist.get(0).getId());
            validateCodeDoMapperExt.updateByPrimaryKeySelective(record);
        } else {
            validateCodeDoMapperExt.insertSelective(record);
        }
        // 发送短信
        try {
            // ShortMessageService.seneSms(mobile, code);
    ...
    private boolean isCodeMatches(String account, String validateCode) {
        ValidateCodeDoExample example = new ValidateCodeDoExample();
        example.createCriteria().andCodeEqualTo(validateCode).andPhoneEqualTo(account).andIsDeletedEqualTo("n");
        List<ValidateCodeDo> rslist = validateCodeDoMapperExt.selectByExample(example);
        if (rslist.size() > 0) {
            Long lastModifiedTime = rslist.get(0).getGmtModified().getTime();
            if (new Date().getTime() - lastModifiedTime <= VALIDTIME) {
                return true;
            }
        }
        return false;
